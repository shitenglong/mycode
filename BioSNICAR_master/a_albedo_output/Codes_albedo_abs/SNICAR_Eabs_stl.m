function result=SNICAR_Eabs_stl(Diameter_core_in,Diameter_OC_in,shell_ratio_in,...
...
m_BC=complex(1.95,0.79);   %@@@@@@@@@@@@@@@@@@@@@@@@@@@调试1.95 0.79  1.85 0.71
...
%                 data_spec_shell_averaged(:,n)=data_spec_shell_output(1:5,3);
            ...
            if abs==1
                eval(['save E:\2018ACP_pw\ACP\SNICAR\snicar_result\OC_shell\305_5000_spec_all_' name '.txt data_out_shell -ascii;'] );%@@@@@@@@@@@@@@@@@
            else
...
%                 data_out_core1_averaged(:,n1)=data_spec_core_output1(1:5,3);
            ...
            if abs==1
                eval(['save E:\2018ACP_pw\ACP\SNICAR\snicar_result\BC_OC\305_5000_spec_all_' name '.txt data_out_core1 -ascii;'] );
            else
                eval(['save E:\2018ACP_pw\ACP\SNICAR\snicar_result\BC_OCnonabs\305_5000_spec_all_' name '.txt data_out_core1 -ascii;'] );
            end
...
%======================================================================================================================================
   ...
   for m2=1:m_nu(2);
            for n2=1:n_nu(2);
                data_spec_core_output=snicar8d_pw_stl(200,snow_radius(m2),coszen,BC_conc(n2),1,data_bc_core_spec_in,0,data_bc_OC_spec_in);
...
%                 data_out_core_averaged(:,n2)=data_spec_core_output(1:5,3);
            ...
            eval(['save E:\2018ACP_pw\ACP\SNICAR\snicar_result\BC_OC\305_5000_spec_all_' name '.txt data_out_core -ascii;'] ); %@@@@@@@@@@@@@@@@@@
            eval(['save E:\2018ACP_pw\ACP\SNICAR\snicar1.5\BC\305_5000_spec_all_' name '.txt data_out_core -ascii;'] ); 
...
%             eval(['save E:\2018ACP_pw\ACP\SNICAR\snicar_result\BC_OC\305_5000_spec_all_' name '.txt data_out_core -ascii;'] ); %@@@@@@@@@@@@@@@@@@
    ...
    if abs==1
        eval(['save E:\2018ACP_pw\ACP\SNICAR\mie_result\OC_shell\305_5000_Eabs_all_' name '.txt data_Eabs_otput -ascii;']);
    else
...
% k_OC_all=load(['E:\2018ACP_pw\BioSNICAR_master\305_5000_k_brown_oc.txt']);
% k_OC_all(k_OC_all<10^-6)=10^-6; 
% lamda=55;
% k_OC_all=1.4034-(6.49e-3)*lamda+(1.0126e-5)*lamda^2-(5.3939e-9)*lamda^3
% m_OC=complex(1.55,0.03); % k_OC_all(26)*1.5
%    m_BC=complex(1.85,0.71);
%    Diameter_core=10:10:1000;
%    for i=1:100;
%        Dx_wave_core=Diameter_core(i)*pi/550; %[qext qsca qabs qb asy qratio];
%        result_core_BC=Mie(m_OC,Dx_wave_core);
%        Cmee_core_BC(i)=result_core_BC(3)*pi*power(Diameter_core(i),2)/4/(pi*Diameter_core(i)^3/6.0*1.2)*10^3;%  6
%    end
