...
% mss_cnc_sot1: mass mixing ratio of black carbon species 1 (uncoated
% BC)黑碳浓度
...
% function data_out = snicar8d_pw(BND_TYP_IN, DIRECT_IN, APRX_TYP_IN, ...
...
function data_out = snicar8d_pw_stl_v( ...
                             rho_snw_in, rds_snw_in, ...
                             coszen_in,mss_cnc_sot1_in,DIRECT_IN,data_bc_optic_in,mss_cnc_OC_in,data_oc_optic_in)
                             %snow_thickness, snow_density, snow_size,cos(Solar_zenith), BC_conc
...
%     rho_snw(1:nbr_lyr) = 150;  %需要修改
  rho_snw(1:nbr_lyr) = rho_snw_in;  %需要修改

    % SNOW GRAIN SIZE FOR EACH LAYER (units: microns):
%     rds_snw(1:nbr_lyr) = 100;  %需要修改
   rds_snw(1:nbr_lyr) = rds_snw_in;  %需要修改
...
%     rds_coated(1:nbr_lyr) = 100;  %需要修改 与 rds_snw保持一致
    rds_coated(1:nbr_lyr) = rds_snw_in;  %需要修改 与 rds_snw保持一致
...
%     mss_cnc_sot1(1:nbr_lyr)  = 100.0;  % uncoated black carbon
    mss_cnc_sot1(1:nbr_lyr)  =mss_cnc_sot1_in; %需要修改
    ...
    mss_cnc_ash1(1:nbr_lyr)  = mss_cnc_OC_in;    % volcanic ash species 1
...
if (BND_TYP==1);
    %wrkdir='./';
    wrkdir=' E:\2018ACP_pw\BioSNICAR_master\Ice_RIs/';%pw
elseif(BND_TYP==2);
...
if (BND_TYP==1);
    ...
    if (DIRECT == 1);
        ...
        load E:\2018ACP_pw\BioSNICAR_master\Misc_Data\mlw_sfc_flx_frc_clr.txt;%pw
    ...
    elseif (DIRECT == 0)
        ...
        load E:\2018ACP_pw\BioSNICAR_master\BioSNICAR_master\Misc_Data\mlw_sfc_flx_frc_cld.txt;
...
omega_aer(:,1)=data_bc_optic_in(:,1);%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@为BC
g_aer(:,1)=data_bc_optic_in(:,2);
ext_cff_mss_aer(:,1)=data_bc_optic_in(:,3);
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
omega_aer(:,7)=data_oc_optic_in(:,1);%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@为OC
g_aer(:,7)=data_oc_optic_in(:,2);
ext_cff_mss_aer(:,7)=data_oc_optic_in(:,3); %@@@@@@@@@@@@@@@@@@@
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
alb_vis = sum(flx_slr(1:vis_max_idx).*albedo(1:vis_max_idx))/...   %@@@@@@@@@@修改为355-695@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
if (BND_TYP == 1);
    ...
    bnd8a=90;
    bnd8b=470;
    bnd9a=110;%@@@@@@@@@@@@1395  stladd
  
    alb1 = sum(flx_slr(bnd1a:bnd2b).*albedo(bnd1a:bnd2b))/sum(flx_slr(bnd1a:bnd2b));%@@@@@@@@@@@@@输出
    omgea1=sum(flx_slr(bnd1a:bnd2b).*omega_star(bnd1a:bnd2b))/sum(flx_slr(bnd1a:bnd2b));%@@@@stladd
    alb2 = sum(flx_slr(bnd2a:bnd2b).*albedo(bnd2a:bnd2b))/sum(flx_slr(bnd2a:bnd2b));%@@@@@@@@@@@@@输出
    omgea2 = sum(flx_slr(bnd2a:bnd2b).*omega_star(bnd2a:bnd2b))/sum(flx_slr(bnd2a:bnd2b));
    ...
    alb_spec355_1395=sum(flx_slr(bnd1a:bnd4b).*albedo(bnd1a:bnd4b))/sum(flx_slr(bnd1a:bnd4b));%@@@@@@@@@@@@@stladd
...
% Output diagnostics:
%  1. The basics:
data_out(:,1) = wvl;            % spectral wavelength bands (um)@@@@@@@@@@@@@@@@@@@输出参数
data_out(:,2) =omega_star;% albedo;         % spectral hemispheric albedo
data_out(1,3) = alb1;            % solar broadband albedo@@@@@@@@@@@@@@@@@@@@@@@@stl修改visible (0.35-0.7um) albedo
data_out(2,3) = alb2;         % visible (0.3-0.7um) albedo@@@@@@@@@@@ near-IR (0.7-1.0um) albedo
data_out(3,3) = alb_spec355_1395;        % near-IR (0.7-5.0um) albedo@@@@@@@@@@@@near-IR (0.35-1.4um) albedo
data_out(4,3) = omgea1; % total radiative absorption by all snow layers (not including underlying substrate)
data_out(5,3) = omgea2;   % top layer solar absorption
if (nbr_lyr > 1);
